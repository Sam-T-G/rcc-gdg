...
| File | What it is |
|---|---|
| `index.html` | The page: an opening headline, then one pinned scroll scene where the semester's staircase draws itself while the Voice chapter works an example answer from 20 to 12 to 8 seconds, then Ask and Evidence, then the next session and the Discord button. The example comes in four majors the visitor picks between. Styles come from `tokens.css`; rules it does not ship are marked PROPOSED |
...
- **The major picker works without scripts.** Four radios drive it through `:has()`, so reduced motion and a blocked CDN still get a working picker and the chosen major's three rounds; without `:has()` all four sets show, each under its own heading. A radio group is one tab stop and arrow keys move inside it.
...
- Checked 2026-09-17. At 320, 390, 768, and 1280 px with the system theme light and dark: horizontal lockup everywhere, no horizontal scroll, every text pair 5.18:1 or better, no tap target under 44 px, zero axe-core 4.10.2 violations, skip link first. Through the scroll scene at 390 x 844, 375 x 667, 1024 x 768, and 1440 x 900: the chapters hand off in order, scrolling back rebuilds the full answer, and the drawn staircase never crosses text. For all four majors at 390 x 844, 375 x 667, and 1440 x 900: the animated line matches that major's written round word for word at 20, 12, and 8, the reserved answer box is the same height for every major so picking one never moves the page, and arrow keys select and re-render with a visible ring on the chip. Static fallback checked with reduced motion and with cdnjs blocked, picker included.
...
Last reviewed: 2026-09-17. Owner: `LEAD`.
