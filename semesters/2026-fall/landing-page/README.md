...
| File | What it is |
|---|---|
| `index.html` | The page: headline, the semester as a green staircase, when and where, one button. Styles come from `tokens.css`; the page layer marks the few rules `tokens.css` does not ship yet as PROPOSED |
...
- **One idea per block, one button.** The staircase is the only decoration: the three arcs as three treads, each label on its own step, drawn once on load and still under reduced motion.
